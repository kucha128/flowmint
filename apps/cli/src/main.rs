...
use cli::{
    ApiCmd, CaptureCmd, CertCmd, Cli, Command, ExportCmd, FlowCmd, McpCmd, RuleCmd,
};
...
async fn main() -> Result<()> {
    ...
    let cli = Cli::parse();
    let engine = Engine::open(&cli.data)
        .with_context(|| format!("打开数据目录 {}", cli.data.display()))?;

    match cli.command {
        Command::Capture(CaptureCmd::Start { port, label, mitm, insecure_upstream, upstream }) => {
            commands::capture::start(&engine, &cli.data, port, label, mitm, insecure_upstream, upstream).await?
        }
        Command::Flow(FlowCmd::Search { host, limit }) => commands::flow::search(&engine, host, limit)?,
        ...
        Command::Api(ApiCmd::Serve { port, token }) => commands::api::serve(engine, port, token).await?,
    }
    Ok(())
}
