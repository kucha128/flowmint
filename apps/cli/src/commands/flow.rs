...
pub fn search(engine: &Engine, host: Option<String>, limit: usize) -> Result<()> {
    let flows = engine.search_flows(&SearchFilter { host, capture_id: None, limit })?;
    if flows.is_empty() {
        println!("(无 flow)");
    }
    for f in &flows {
        println!(
            "{}  {:<6} {:<3} {}{}",
            f.flow_id,
            f.method.as_deref().unwrap_or(f.l7.as_deref().unwrap_or("-")),
            f.status.map(|s| s.to_string()).unwrap_or_else(|| "-".into()),
...
pub fn show(engine: &Engine, flow_id: &str) -> Result<()> {
    ...
    for ev in engine.events_for_flow(flow_id)? {
        let bytes = match &ev.payload {
            Some(p) => format!(" payload={}B sha={}", p.length, &p.sha256[..8.min(p.sha256.len())]),
            None => String::new(),
        };
        println!("  #{} {:?} {:?}{}", ev.sequence, ev.direction, ev.kind, bytes);
    }
    Ok(())
}
