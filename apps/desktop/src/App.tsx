...
import {
  ...
  installCa, isCaInstalled, onBreakpoint, onFlow, onWsFrame, regenerateCa, resumeBreakpoint,
  searchFlows, setBreakpoints, setConfig, setSystemProxy, startCapture, stopCapture, type Flow,
  type FlowDetail, type PausedMessage,
} from "./lib/api";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // 实时 WS 帧：属于当前打开会话就追加（按 event_id 去重，避免与首拉快照重复）。
    onWsFrame((fr) => {
      setDetail((prev) => {
        if (!prev || prev.flow.flow_id !== fr.flow_id) return prev;
        if (prev.events.some((e) => e.event_id === fr.event_id)) return prev;
        return { ...prev, events: [...prev.events, fr] };
      });
    }).then((u) => unlisteners.push(u));
