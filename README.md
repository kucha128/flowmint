...
设计北极星（优先级从高到低）：**使用方便 → 分析效率高 → 分析完接入开发简单**。
...
> **首次用 HTTPS 解密无需预置证书**：MITM 的 CA 会在你本机首次用到时**自动生成**（存于 `<exe目录>/data/ca/`），每台机器各不相同、私钥仅本地保留。第一次点「安装证书」或开 MITM 抓包即生成你自己的 CA，随后在本机信任它即可。⚠️ 切勿把 `data/ca/`（含私钥）外传或提交，发布 release 时也只发 exe、不含 `data/`。
